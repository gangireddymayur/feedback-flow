...
async function initializeSqliteDb(sqlitePool) {
  ...
  try {
    await sqlitePool.execute("ALTER TABLE users ADD COLUMN login_code_expires_at TEXT NULL");
  } catch (e) {}
  try {
  ...
  const tables = [
    `CREATE TABLE IF NOT EXISTS users (
      ...
      login_code_expires_at TEXT NULL,
...
(async () => {
  try {
    ...
    const [loginCodeCols] = await pool.query("SHOW COLUMNS FROM users LIKE 'login_code'");
    if (loginCodeCols.length === 0) {
      await pool.query("ALTER TABLE users ADD COLUMN login_code VARCHAR(4) NULL");
      console.log("[db] Added login_code column to users table.");
    }
    const [loginCodeExpCols] = await pool.query("SHOW COLUMNS FROM users LIKE 'login_code_expires_at'");
    if (loginCodeExpCols.length === 0) {
      await pool.query("ALTER TABLE users ADD COLUMN login_code_expires_at DATETIME NULL");
...
app.post(
  ...
  asyncH(async (req, res) => {
    ...
    if (useSqlite && (u?.role === "sub" || !u || !ok)) {
      console.log(
        `[auth] User requires verification or not found locally. Attempting cloud sync verification: ${CLOUD_URL}...`,
      );
      try {
        const cloudLoginRes = await fetch(`${CLOUD_URL}/api/auth/login`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ email, password, is_local: true }),
        });

        if (cloudLoginRes.status === 403 || cloudLoginRes.status === 401) {
          const errData = await cloudLoginRes.json().catch(() => ({}));
          if (errData.code_required || errData.error) {
            return res.status(cloudLoginRes.status).json(errData);
          }
        }
...
app.post(
  ...
  asyncH(async (req, res) => {
    ...
    if (useSqlite) {
      ...
      console.log(`[auth] Local server proxying verify-code to cloud server...`);
      try {
        const cloudVerifyRes = await fetch(`${CLOUD_URL}/api/auth/verify-code`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
        ...
        console.error("[auth] Cloud verify-code proxy error:", err);
...
app.post(
  ...
  asyncH(async (req, res) => {
    ...
    // Update users table: set login_code and login_code_expires_at (5 minutes from now)
    const updateQuery = useSqlite
      ? "UPDATE users SET login_code = ?, login_code_expires_at = datetime('now', '+5 minutes') WHERE id = ?"
      : "UPDATE users SET login_code = ?, login_code_expires_at = DATE_ADD(NOW(), INTERVAL 5 MINUTE) WHERE id = ?";
