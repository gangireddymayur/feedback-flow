...
function SuperDashboard() {
  ...
  const devicesQ = useQuery({ queryKey: ["devices"], queryFn: () => Devices.list() });
  const responsesQ = useQuery({ queryKey: ["responses"], queryFn: () => Responses.list(), refetchInterval: 30000 });
  const templatesQ = useQuery({ queryKey: ["templates"], queryFn: () => Templates.list() });
  ...
  return (
    <DashboardLayout>
      <PageHeader
        ...
        actions={
          <Button asChild>
            <Link to="/admins"><Users className="size-4" /> Manage Sub Admins</Link>
          </Button>
        }
      />
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <Kpi icon={FileText} label="Templates in use" value={templates.length.toString()} tone="success" />
        <Kpi icon={Smartphone} label="Devices deployed" value={devices.length.toString()} tone="warn" />
        <Kpi icon={MessageSquare} label="Total Reviews" value={totalReviews.toLocaleString()} />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          ...
          <div className="h-64">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={trend} margin={{ left: -20, right: 0, top: 10 }}>
                ...
                <YAxis stroke="oklch(0.7 0.025 255)" fontSize={12} tickLine={false} axisLine={false} />
                <Tooltip contentStyle={{ background: "oklch(0.22 0.035 260 / 0.9)", border: "1px solid oklch(1 0 0 / 0.1)", borderRadius: 12 }} />
                <Area type="monotone" dataKey="responses" stroke="oklch(0.78 0.18 300)" strokeWidth={2} fill="url(#g2)" />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </GlassCard>
        ...
        <GlassCard>
          <div className="flex items-center justify-between mb-4">
            ...
            <Link to="/admins" className="text-xs text-primary inline-flex items-center gap-1 hover:underline">
              Manage <ArrowUpRight className="size-3" />
            </Link>
          </div>
          <div className="space-y-3 max-h-64 overflow-auto pr-1">
            {subAdmins.length === 0 && <div className="text-xs text-muted-foreground">No sub-admins yet.</div>}
            {[...subAdmins].sort((a, b) => b.devices - a.devices).slice(0, 6).map((a) => (
      ...
      <GlassCard>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <StatRow label="Active templates" value={templates.filter((t) => t.status === "active").length} total={templates.length} />
          <StatRow label="Online devices" value={devices.filter((d) => d.status === "online").length} total={devices.length} />
